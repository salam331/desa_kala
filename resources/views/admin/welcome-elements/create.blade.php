...
@section('title', 'Tambah Konten Welcome Baru')
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        <h3 class="text-sm font-medium text-amber-800 dark:text-amber-200">
                            Perhatian: Pilih Bagian dengan Benar
                        </h3>
                        <div class="mt-2 text-sm text-amber-700 dark:text-amber-300">
                            <p>Pastikan memilih bagian halaman yang tepat agar konten muncul di tempat yang benar. Jika ragu, lihat bagian mana yang ingin diubah di halaman depan website.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.welcome-elements.store') }}">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500">Pilih bagian halaman dimana konten ini akan ditampilkan</p>
                                @error('element_type')
                            ...
                            <div>
                                ...
                                <input type="text" id="element_key" name="element_key" value="{{ old('element_key') }}"
                                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                       placeholder="contoh: title, description, button_text">
                                <p class="mt-1 text-sm text-gray-500">Masukkan nama unik untuk jenis konten ini</p>
                                @error('element_key')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', 1) }}"
                                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <p class="mt-1 text-sm text-gray-500">Urutan tampilan konten (angka kecil akan tampil lebih dulu)</p>
                                @error('sort_order')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500">Konten yang tidak aktif tidak akan tampil di halaman website</p>
                                @error('is_active')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea id="content" name="content" rows="6"
                                      class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                      placeholder="Masukkan isi konten yang akan ditampilkan di halaman website">{{ old('content') }}</textarea>
                            <p class="mt-1 text-sm text-gray-500">Tuliskan konten yang akan tampil di halaman depan website Desa Kala</p>
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
