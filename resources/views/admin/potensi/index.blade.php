                    ...
                    @forelse($potensi as $item)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                {{ $item->kontak ?: '-' }}
                                @if($item->telepon)
                                    <br><small class="text-gray-500 dark:text-gray-300">{{ $item->telepon }}</small>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                {{ $item->lokasi ?: '-' }}
    ...
    <div class="block md:hidden space-y-4">
        @forelse($potensi as $item)
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                        ...
                        <div class="grid grid-cols-2 gap-2 text-sm">
                            <div>
                                <span class="font-medium text-gray-500 dark:text-gray-400">Kontak:</span>
                                <span class="text-gray-900 dark:text-gray-100">{{ $item->kontak ?: '-' }}</span>
                                @if($item->telepon)
                                    <br><span class="text-gray-500 dark:text-gray-300">{{ $item->telepon }}</span>
                                @endif
                            </div>
                            <div>
                                <span class="font-medium text-gray-500 dark:text-gray-400">Lokasi:</span>
                                <span class="text-gray-900 dark:text-gray-100">{{ $item->lokasi ?: '-' }}</span>
                            </div>
                        </div>
                        <div class="mt-2">
                            @if($item->is_active)
                ...
                <div class="flex space-x-2 mt-4">
                    ...
                    <form method="POST" action="{{ route('admin.potensi.destroy', $item->id) }}"
                          class="flex-1"
                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus potensi ini?')">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <div class="text-center py-12">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">Belum ada potensi yang ditambahkan</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Mulai tambahkan potensi desa untuk memperkenalkan kekayaan desa Anda.</p>
                <div class="mt-6">
                    <a href="{{ route('admin.potensi.create') }}"
